...
public class SearchOperation : MonoBehaviour    // 搜寻目标
{
    ...
    //AdjacencyMatrix adjacencyMatrix = AdjacencyMatrixManager.Instance.adjacencyMatrix;
    ...
    public List<plateData> SearchSetArmy(string nationName, List<plateData> plates, string campusName)
    {
        ...
        SearchMethod(searchById, nationName, campusName, plates);
        canSetPlate(isSetArmy, searchById, campusName, plates);
        isSetArmy = isSetArmy.Where(plate => plate.plateType == "Land").ToList();
        return isSetArmy;
    }

    // 陆地攻击判断
    public List<plateData> SearchAttackArmy(string nationName, string campusName, List<plateData> plates)
    {
        adjacencyMatrix = AdjacencyMatrix.Instance.GetAdjacencyMatrix();
        List<plateData> isAttackedArmy = new List<plateData>(); // 可被攻击的陆军
        List<int> searchById = new List<int>(); // 存放的要查找后的板块id
        SearchMethod(searchById, nationName, campusName, plates);
        canAttackPlate(isAttackedArmy, searchById, campusName, plates);
        isAttackedArmy = isAttackedArmy.Where(plate => plate.plateType == "Land").ToList();
    ...
    public List<plateData> SearchSetNavy(string nationName, List<plateData> plates, string campusName)
    {
        ...
        List<int> searchById = new List<int>(); // 存放的要查找后的板块id
        SearchMethod(searchById, nationName, campusName, plates);
        canSetPlate(isSetNavy, searchById, campusName, plates);
        isSetNavy = isSetNavy.Where(plate => plate.plateType == "Sea").ToList();
        return isSetNavy;
    }

    // 海洋攻击判断
    public List<plateData> SearchAttackNavy(string nationName, string campusName, List<plateData> plates)
    {
        adjacencyMatrix = AdjacencyMatrix.Instance.GetAdjacencyMatrix();
        List<plateData> isAttackedNavy = new List<plateData>(); // 可被攻击的海军
        List<int> searchById = new List<int>(); // 存放的要查找后的板块id
        SearchMethod(searchById, nationName, campusName, plates);
        canAttackPlate(isAttackedNavy, searchById, campusName, plates);
        isAttackedNavy = isAttackedNavy.Where(plate => plate.plateType == "Sea").ToList();
        return isAttackedNavy;
    }

    public void SearchMethod(List<int> searchById, string nationName, string campusName, List<plateData> plates)
    {
        adjacencyMatrix = AdjacencyMatrix.Instance.GetAdjacencyMatrix();
        ...
        {
            if ((plate.takeStatuByCountry1.Equals(nationName) || plate.takeStatuByCountry1.Equals(nationName) || plate.takeStatuByCountry3.Equals(nationName)) && plate.isInSupply == true)
            {
    ...
    private void canSetPlate(List<plateData> returnList, List<int> searchById, string campusName, List<plateData> plates)
    {
        adjacencyMatrix = AdjacencyMatrix.Instance.GetAdjacencyMatrix();
        foreach (int id in searchById)
        {
            foreach (plateData plate in plates)
            {
                if (plate.plateId == id && (plate.takeStatuByCampus.Equals(campusName) || plate.takeStatuByCampus == ""))
                {
    ...
    private void canAttackPlate(List<plateData> returnList, List<int> searchById, string campusName, List<plateData> plates)
    {
        adjacencyMatrix = AdjacencyMatrix.Instance.GetAdjacencyMatrix();
        foreach (int id in searchById)
        {
